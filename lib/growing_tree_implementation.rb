...
# (pull the oldest cell), or a combination of any of these, e.g.:p
#
...
class Script
  ...
  def parse_command(cmd)
    ...
    parts = cmd.split(/,/).map do |element|
      ...
      weight ||= 100
      abort "commands must be random, newest, middle, or oldest (was #{name.inspect})" unless %w[random r newest n
                                                                                                 middle m oldest o].include?(name)
  ...
  def to_s
    @commands.map do |command|
      v = 0
      command[:parts].map do |part|
        s = "#{part[:name]}:#{part[:weight] - v}"
        v = part[:weight]
        s
      end.join(',')
    end.join(';')
...
until cells.empty?
  ...
  [N, S, E, W].shuffle.each do |dir|
    nx = x + DX[dir]
    ny = y + DY[dir]
    next unless nx >= 0 && ny >= 0 && nx < width && ny < height && grid[ny][nx] == 0
